  ...
  const renderLink = (link: typeof navLinks[0], isMobile: boolean = false) => {
    ...
    if (link.type === 'link' && link.path) {
        ...
        return (
            ...
            >
                {link.text}
                <span className={`absolute bottom-0 left-0 h-0.5 bg-gradient-to-r from-blue-400 to-teal-400 transition-all duration-300 ${isMobile ? 'left-3 group-hover:w-[calc(100%-1.5rem)]' : 'group-hover:w-full'} ${isActive ? (isMobile ? 'w-[calc(100%-1.5rem)]' : 'w-full') : 'w-0'}`}></span>
            </Link>
        );
    }
    ...
    return (
        ...
        >
            {link.text}
            <span className={`absolute bottom-0 left-0 w-0 h-0.5 bg-gradient-to-r from-blue-400 to-teal-400 transition-all duration-300 ${isMobile ? 'left-3 group-hover:w-[calc(100%-1.5rem)]' : 'group-hover:w-full'}`}></span>
        </a>
    );
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 ">
            ...
            <div className="flex-shrink-0">
              <a href="/#" onClick={(e) => { e.preventDefault(); handleLinkClick("home"); }} className="cursor-pointer">
            ...
            <div className="hidden md:flex items-center space-x-4">
              ...
              <a href="/signup" target="_blank" rel="noopener noreferrer" className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded-md text-sm font-medium transition-transform hover:scale-105">
                Signup
              </a>
              <a href="/login" target="_blank" rel="noopener noreferrer" className="text-gray-300 bg-gray-700 hover:bg-gray-600 hover:text-white px-3 py-2 rounded-md text-sm font-medium transition-colors">
                Login
              </a>
            </div>
            ...
            <div className="-mr-2 flex md:hidden">
              <button onClick={() => setIsMenuOpen(!isMenuOpen)} type="button" className="bg-gray-800 inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-white hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" aria-controls="mobile-menu" aria-expanded={isMenuOpen}>
                ...
                <svg className={`${isMenuOpen ? "hidden" : "block"} h-6 w-6`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16m-7 6h7" /></svg>
        ...
        <div className={`${isMenuOpen ? "block" : "hidden"} md:hidden border-t border-gray-700`} id="mobile-menu">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            <div className="pt-4 border-t border-gray-700 mt-4">
              <a href="/signup" target="_blank" rel="noopener noreferrer" className="block text-center bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded-md text-base font-medium transition-colors mb-2">
                Signup
              </a>
              <a href="/login" target="_blank" rel="noopener noreferrer" className="block text-center text-gray-300 bg-gray-700 hover:bg-gray-600 hover:text-white px-3 py-2 rounded-md text-base font-medium transition-colors">
                Login
              </a>
            </div>
          </div>
